...
def search_hotel(idHotel, start_date, end_date):
...
def FindHotel():
  ...
  for i in range(14):
    ...
    res = search_hotel(idHotel, start_date, end_date)["data"]["citySearch"]["properties"]
    ...
    if(len(theHotel) > 0):
        ...
        if(len(theHotel[0]["pricing"]["offers"]) > 0):
          price = theHotel[0].get("pricing", {}).get("offers", [])[0].get("roomOffers", [])[0].get("room", {}).get("mseRoomSummaries", [])[0].get("pricingSummaries", [])[0].get("price", {}).get("perRoomPerNight")
        list.append({
            'name': nameHotel,
            "id": idHotel,
            "date": start_date,
